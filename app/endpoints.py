...
async def handle_incoming_call(request: Request):
    ...
    shared.global_from_number = from_number
    shared.global_to_number = to_number
    call_sid = form_data.get("CallSid")
    if not call_sid:
        call_sid = f"{int(datetime.datetime.utcnow().timestamp())}"
    shared.global_call_id = call_sid
...
async def media_stream(websocket: WebSocket):
    print("voici le numero", shared.global_to_number)
    system_message = await get_prompt_for_number(shared.global_to_number)
